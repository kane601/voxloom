...
This feature exposes OpenAI-compatible endpoints from VoxLoom, allowing any tool, library, or application that speaks the OpenAI Audio API to use VoxLoom as a drop-in local replacement.
...
flowchart LR
    ...
    subgraph voxloom [VoxLoom Server]
...
- **OpenAI SDK users**: `openai.audio.speech.create()` works with VoxLoom
- **LLM frameworks**: LangChain, AutoGen, etc. can use VoxLoom for TTS
...
- `voice` parameter maps to VoxLoom profile names (case-insensitive)
...
async def resolve_voice_for_openai(voice: str, db: Session) -> Optional[VoiceProfile]:
    """
    Resolve OpenAI voice parameter to a VoxLoom profile.
