...
VoxLoom ships a built-in **Model Context Protocol** server so local AI
agents can call your VoxLoom install directly: speak text in a voice
profile, transcribe audio, and list captures or profiles. The server runs
inside the same process as the rest of VoxLoom and is mounted at `/mcp`
over Streamable HTTP.

Agent asks to speak → VoxLoom plays audio on your speakers → an on-screen
...
VoxLoom → Settings → MCP. The value is just an identifier for the
per-client voice binding — not a secret, not a credential.

### Clients that only speak stdio

A stdio shim binary `voxloom-mcp` is bundled with the desktop app. Point
...
<Tabs items={["macOS", "Windows", "Linux"]}>
  ...
  ```json
  {
    "mcpServers": {
      "voxloom": {
        "command": "C:\\Program Files\\VoxLoom\\voxloom-mcp.exe",
        "env": { "VOXLOOM_CLIENT_ID": "claude-desktop" }
      }
    }
  }
  ...
  ```json
  {
    "mcpServers": {
      "voxloom": {
        "command": "/opt/voxloom/voxloom-mcp",
        "env": { "VOXLOOM_CLIENT_ID": "claude-desktop" }
      }
    }
  }
...
The shim waits up to 30 seconds for the VoxLoom backend to come up, then
proxies JSON-RPC from stdio over Streamable HTTP. VoxLoom must be running
...
| Tool | Use |
|---|---|
| `voxloom.speak` | Speak text in a voice profile. Returns a `generation_id` to poll. |
| `voxloom.transcribe` | Whisper transcription of base64 audio or an absolute local path. |
| `voxloom.list_captures` | Recent captures with transcripts, paginated. |
...
Every call to `voxloom.speak` (and `POST /speak`) resolves the voice profile
in this order:
...
<Steps>
  ...
  <Step title="Per-client binding">
    Looked up by the `X-VoxLoom-Client-Id` header. Managed in
    **VoxLoom → Settings → MCP**. Lets you pin Claude Code to Morgan,
...
VoxLoom → Settings → MCP shows one row per `client_id` VoxLoom has heard
...
| `default_personality` | When true, `voxloom.speak` routes through the profile's personality LLM (rewrite) by default. |
...
silent background TTS is a trust hazard, so VoxLoom always shows what's
...
Start with `voxloom.list_profiles` to confirm wiring, then
`voxloom.speak` for end-to-end — you should hear audio and see the
generation land in the Captures tab.

<Callout type="info">
  If an agent can't reach the server, the first thing to check is that
  VoxLoom is running — the backend only listens while the desktop app is
...
- **Localhost only.** The server binds to `127.0.0.1`. If you ever point
  VoxLoom at a non-loopback interface (e.g. remote-mode over a trusted
...
- **No auth today.** Any process that can connect to your loopback can
  call MCP. That's the same trust boundary as the rest of VoxLoom's REST
...
- **Voice cloning consent applies.** See [Voice Cloning](/overview/voice-cloning#limitations)
  — an agent being able to call `voxloom.speak` in someone's voice
...
- **Lifespan:** mounting FastMCP requires the `lifespan=` kwarg on
  ...
  with FastMCP's Streamable HTTP session manager. The VoxLoom app.py
