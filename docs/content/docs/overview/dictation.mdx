...
a chord, talk, release — VoxLoom transcribes what you said with Whisper,
...
<Callout type="info">
  ...
  per-profile personality modes. It's the "input" half of VoxLoom's voice I/O
...
<Steps>
  ...
  <Step title="Text lands in your app">
    If auto-paste is enabled and VoxLoom has Accessibility permission, the
...
VoxLoom ships two chord behaviors out of the box:
...
- **Left vs right modifier badges.** When you hold keys into the chord
  picker, VoxLoom records whether each modifier is the left or right variant.
...
Once transcription finishes, VoxLoom can synthesize a native paste into
...
- Whisper loop hallucinations — VoxLoom strips repeated tokens (six or
...
- **UAC / UIPI caveat.** Synthetic paste into an *elevated* window from a
  non-elevated VoxLoom is blocked by Windows itself. Run VoxLoom elevated
...
A few cases where VoxLoom deliberately does *not* synthesize a paste:

- **Focus was inside VoxLoom** when the chord started. The transcript goes
  to the Captures tab so a dictation-into-VoxLoom round-trip doesn't
