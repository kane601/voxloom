...
description: "Run VoxLoom as a headless server with a web UI using Docker"
---

## Overview

VoxLoom can run as a Docker container with a full web UI -- no desktop app required. This is ideal for headless servers, shared GPU machines, or self-hosted deployments.
...
Open [http://localhost:17493](http://localhost:17493) in your browser. The full VoxLoom UI is served directly from the backend.
...
services:
  voxloom:
    ...
    volumes:
      ...
      - huggingface-cache:/home/voxloom/.cache/huggingface
    environment:
      - LOG_LEVEL=info
    deploy:
      resources:
...
| `VOXLOOM_MODELS_DIR` | (HuggingFace cache) | Custom path for model storage |
| `VOXLOOM_CORS_ORIGINS` | (local origins) | Additional CORS origins, comma-separated |
...
| `voxloom-data` | `/app/data` | Profiles, database, cache |
| `huggingface-cache` | `/home/voxloom/.cache/huggingface` | Downloaded models (persists across rebuilds) |
...
- **Non-root user** -- the server runs as `voxloom`, not `root`
...
For production deployments, put VoxLoom behind nginx or Caddy with TLS and authentication:

```nginx
server {
    listen 443 ssl;
    server_name voxloom.example.com;

    ssl_certificate /etc/ssl/certs/voxloom.pem;
    ssl_certificate_key /etc/ssl/private/voxloom.key;
...
If you see `{"message": "voxloom API", ...}` instead of the web UI, the frontend build may have failed during the Docker build. Check the build logs:
...
docker run -p 17493:17493 ghcr.io/jamiepine/voxloom:latest
```
