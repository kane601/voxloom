...
description: "Common issues and solutions for VoxLoom"
---

This guide covers common issues you might encounter when using or developing VoxLoom, along with solutions.
...
xattr -cr /Applications/VoxLoom.app
```
...
<AccordionGroup>
  ...
  <Accordion title="Permission Issues">
    ...
    ```bash
    # macOS/Linux
    chmod +x ~/Library/Application\ Support/sh.voxloom.app/backend/voxloom-server
    ```
  </Accordion>
  ...
  <Accordion title="Check Logs">
    ...
    **macOS:**
    ```bash
    tail -f ~/Library/Application\ Support/sh.voxloom.app/logs/server.log
    ```

    **Windows:**
    ```bash
    type %APPDATA%\sh.voxloom.app\logs\server.log
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Install flash-attn on Windows (community wheels)">
    ...
    Alternatively, run VoxLoom's backend inside WSL2 and use the standard Linux wheels.
  </Accordion>
</AccordionGroup>
...
**Solutions:**
- Close all VoxLoom instances
- Delete the lock file:
  ```bash
  # macOS
  rm ~/Library/Application\ Support/sh.voxloom.app/data/voxloom.db-shm
  rm ~/Library/Application\ Support/sh.voxloom.app/data/voxloom.db-wal
  ```
...
```bash
# macOS
rm ~/Library/Application\ Support/sh.voxloom.app/data/voxloom.db

# Windows
del %APPDATA%\sh.voxloom.app\data\voxloom.db
```
...
<AccordionGroup>
  ...
  <Accordion title="Check GPU Selection">
    If you have multiple GPUs, ensure VoxLoom is using the right one.
...
1. **Check GitHub Issues:** [github.com/jamiepine/voxloom/issues](https://github.com/jamiepine/voxloom/issues)
2. **Open a New Issue:** Provide:
   - Operating system and version
   - VoxLoom version
   - Steps to reproduce
   - Error messages or logs
3. **Join Discord:** [discord.gg/voxloom](https://discord.gg/voxloom) (coming soon)
