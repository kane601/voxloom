...
The model runs through the same backend split VoxLoom already uses for TTS
...
<Steps>
  ...
  <Step title="Speak in character — toggle persona rewrite">
    ...
    generate — VoxLoom runs the text through the personality LLM first,
...
- **Agents that speak in a voice you own.** Combine the persona toggle with
  ...
  personality. The agent calls `voxloom.speak({ text, profile, personality:
  true })` and VoxLoom rewrites the text in character before speaking.
...
`voxloom.speak` tool uses when you pass `personality: true`. Scripts and
