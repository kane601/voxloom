---
title: "Building"
description: "How VoxLoom is built for production"
---

## Overview

VoxLoom uses a two-stage build process:
...
cp dist/voxloom-server ../tauri/src-tauri/binaries/voxloom-server-${PLATFORM}
```
...
```json
{
  "tauri": {
    "bundle": {
      "externalBin": ["binaries/voxloom-server"]
    }
  }
}
```

Tauri looks for `voxloom-server-${PLATFORM}` in `src-tauri/binaries/` and bundles it.
...
Signing is **optional** — the workflow auto-detects whether the signing secrets exist and skips signing/notarization when they don't.

**macOS (with secrets configured):**
- Apple Developer certificate imported from secrets
- Notarization via App Store Connect API

**macOS (no secrets):**
- Produces an unsigned, unnotarized DMG. End users must right-click → Open (or allow it in System Settings → Privacy & Security) to run it on macOS 15+.

**Windows (no secrets):**
- Currently produces an unsigned MSI/NSIS by default. SmartScreen shows a warning; users pick "More info → Run anyway".
- To enable automatic signing later, add `WINDOWS_CERTIFICATE`, `WINDOWS_CERTIFICATE_PASSWORD`, and `WINDOWS_CERTIFICATE_THUMBPRINT` secrets.

**Both platforms:** `TAURI_SIGNING_PRIVATE_KEY` / `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` are still required for auto-updater artifacts (`createUpdaterArtifacts`). Generate one with `bun tauri signer generate -w ~/.tauri/voxloom.key`.
...
3. Package with `scripts/package_cuda.py` into two archives:
   - `voxloom-server-cuda.tar.gz` — server core (~945 MB)
...
<AccordionGroup>
  ...
  <Accordion title="Tauri can't find sidecar">
    Ensure binary exists at `tauri/src-tauri/binaries/voxloom-server-${PLATFORM}` before running Tauri build.
  </Accordion>
</AccordionGroup>
