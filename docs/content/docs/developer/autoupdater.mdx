---
title: "Auto-Updater"
description: "How VoxLoom automatic updates work"
---

## Overview

VoxLoom uses Tauri's built-in auto-updater to deliver signed updates to users. The system verifies updates cryptographically before installation.

## How It Works

When VoxLoom launches (in production Tauri builds only), it checks GitHub Releases for a `latest.json` manifest. If a newer version is available:
...
```json
{
  "plugins": {
    "updater": {
      ...
      "dialog": false,
      "endpoints": [
        "https://github.com/jamiepine/voxloom/releases/latest/download/latest.json"
      ],
...
```json
{
  ...
  "platforms": {
    "darwin-aarch64": {
      ...
      "url": "https://github.com/jamiepine/voxloom/releases/download/v0.2.0/voxloom_0.2.0_aarch64.app.tar.gz"
    },
    "darwin-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/jamiepine/voxloom/releases/download/v0.2.0/voxloom_0.2.0_x64.app.tar.gz"
    },
    "linux-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/jamiepine/voxloom/releases/download/v0.2.0/voxloom_0.2.0_amd64.AppImage"
    },
    "windows-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/jamiepine/voxloom/releases/download/v0.2.0/voxloom_0.2.0_x64_en-US.msi"
    }
  }
}
```
...
1. **Generate keys** (one-time setup):
   ```bash
   bun tauri signer generate -w ~/.tauri/voxloom.key
   ```
   This creates:
   - Private key: `~/.tauri/voxloom.key` (stored in GitHub Secrets, never committed)
   - Public key: `~/.tauri/voxloom.key.pub` (pasted into `tauri.conf.json`)
...
- `TAURI_SIGNING_PRIVATE_KEY` - Content of `~/.tauri/voxloom.key`
...
- CUDA server core: ~945 MB (versioned with each VoxLoom release)
...
- **Server core** (`voxloom-server-cuda.tar.gz`) — the Python server + PyTorch code, changes every release.
...
This means most VoxLoom upgrades only re-download the ~945 MB server core, not the full ~2.5 GB bundle.
...
5. **Restart** — The VoxLoom server restarts and swaps in the CUDA backend.
...
{data_dir}/backends/cuda/
  voxloom-server-cuda.exe    # Windows
  voxloom-server-cuda        # macOS/Linux
...
event: progress
data: {"current": 52428800, "total": 945000000, "filename": "voxloom-server-cuda.tar.gz", "status": "downloading"}
```
...
- `voxloom-server-cuda.tar.gz` — server-core archive
- `voxloom-server-cuda.tar.gz.sha256` — checksum
