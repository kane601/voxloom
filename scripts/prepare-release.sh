...
if [ ! -f ~/.tauri/voxloom.key ]; then
  echo "❌ Private key not found at ~/.tauri/voxloom.key"
  echo "Run: cd tauri && bun tauri signer generate -w ~/.tauri/voxloom.key"
  exit 1
fi

if [ ! -f ~/.tauri/voxloom.key.pub ]; then
  echo "❌ Public key not found at ~/.tauri/voxloom.key.pub"
  exit 1
fi
