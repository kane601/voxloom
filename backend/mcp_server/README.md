...
The server runs inside the same `uvicorn` process as the rest of VoxLoom
...
`voxloom-mcp` binary ships inside the VoxLoom.app bundle:
...
| Name | Purpose |
|---|---|
| `voxloom.speak`          | Speak text in a voice profile. Returns a generation id you can poll. |
| `voxloom.transcribe`     | Whisper transcription of a base64 blob or an absolute local path. |
| `voxloom.list_captures`  | Recent captures (dictation / recording / file) with transcripts. |
...
2. Per-client binding keyed by `X-VoxLoom-Client-Id`
...
Point it at the URL, hit "List tools," call `voxloom.list_profiles`
first to confirm wiring, then `voxloom.speak` for end-to-end.
...
curl -X POST http://127.0.0.1:17493/speak \
  ...
  -H 'X-VoxLoom-Client-Id: claude-code' \
