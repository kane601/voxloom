"""Stdio → Streamable HTTP bridge for the VoxLoom MCP server.
...
the client spawns us as ``voxloom-mcp``; we proxy every JSON-RPC frame
to http://127.0.0.1:17493/mcp/ and stream responses back out.

All the real work (tools, models, inference) lives in the VoxLoom server
