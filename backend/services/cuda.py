...
Downloads two archives from GitHub Releases:
  1. Server core (voxloom-server-cuda.tar.gz) — the exe + non-NVIDIA deps,
...
def get_cuda_exe_name() -> str:
    ...
    if sys.platform == "win32":
        return "voxloom-server-cuda.exe"
    return "voxloom-server-cuda"
...
def is_cuda_active() -> bool:
    ...
    """
    return os.environ.get("VOXLOOM_BACKEND_VARIANT") == "cuda"
...
async def _download_cuda_binary_locked(version: Optional[str] = None):
    ...
    server_archive = "voxloom-server-cuda.tar.gz"
